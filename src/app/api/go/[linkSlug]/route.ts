...
export async function GET(
  ...
  // First-party attribution cookie: the click UUID and nothing else. Not
  // httpOnly, so the browser-side analytics layer can correlate an outbound
  // click with the session that produced it.
  response.cookies.set({
    name: CLICK_COOKIE_NAME,
    value: clickId,
    maxAge: CLICK_COOKIE_MAX_AGE_SECONDS,
    path: "/",
    sameSite: "lax",
